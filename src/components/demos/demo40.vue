<template>
  <div class="main">
    ...
    <div class="content-container">
      <el-table :data="tableData" row-key="rowId" :row-class-name="setRowClassName" border @row-dblclick="dbClickHandler" class="table">
        <!-- 拖拽手柄 -->
        <el-table-column width="50">
          <template slot-scope="scope">
            <el-button
              v-if="!isStartOrEndRow(scope.row.rowId)"
        ...
        <el-table-column label="操作" width="100">
          <template slot-scope="scope">
            <template v-if="scope.row.isEditing">
              <el-button @click="saveEdit(scope.$index)" type="text" size="small" :disabled="isRowSaveBtnDisabled">
                保存
              </el-button>
        ...
        <template slot="append">
          <div class="append-row">
            <el-button class="button" @click="insertRow" type="text" :disabled="isRowEditing" icon="el-icon-plus">
              插入
            </el-button>
          </div>
        </template>
