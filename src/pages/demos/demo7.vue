<template>
  <div>
    ...
    <label>
      <input class="add-input" type="text" v-model="newTodo" placeholder="Add a todo" @keyup.enter="addTodo" @keyup.esc="clearInput" />
      <button @click="addTodo">add</button>
      <button @click="clearInput">clear</button>
      <button @click="clearAll">clearAll</button>
    </label>
    <ul class="todo-container" v-if="todoList.length > 0">
      <li is="child" v-for="(todo, index) of todoList" :key="todo.content" :content="todo" @deleteTodo="deleteHandler(index)">
        {{ index + 1 }}.
      </li>
    </ul>
    <ul class="finished-container" v-if="finishedList.length > 0">
      <li v-for="(todo, index) of finishedList" :key="todo.content + 'f'" class="finished-todo">{{ index + 1 }}. {{ todo.content }}</li>
    </ul>
  </div>
</template>
